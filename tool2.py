...
def list_img_file(directory):
    ...
    # print new_list
    new_list.sort() #mac下读取文件可能不按照文件名进行读取，需要重新排序
...
def compress_photo():
    '''调用压缩图片的函数
    '''
    src_dir, des_dir = "design/", "min_design/"
...
def handle_photo():
    ...
    with open("../myblog/source/design/data.json","w") as fp:
...
def cut_photo():
    ...
    if directory_exists(src_dir):
        ...
        if file_list:
            ...
            for infile in file_list:
                img = Image.open(src_dir+infile)
                Graphics(infile=src_dir+infile, outfile=src_dir + infile).cut_by_ratio2()            
        else:
            pass
    else:
...
def git_operation():
    ...
    os.system('git commit -m "add design"')
